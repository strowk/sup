...
#[test]
fn test_stash_and_pop_uncommitted_change_then_commit_with_failing_pre_push_hook_and_then_skip_push_hook() {
    let temp = tempfile::tempdir().unwrap();
    let repo1 = temp.path().join("repo1_bare");
    let repo2 = temp.path().join("repo2");
    // Create bare repo1
    ...
    // but can skip the push hook by passing --no-verify
    run_sup(&repo2, &["-m", "commit message", "--no-verify"], false);
